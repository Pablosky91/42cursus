...
/*   Updated: 2024/09/12 22:24:02 by pdel-olm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static t_error	save_fish(t_game *game, int row, int col);
static t_error	save_penguin(t_game *game, int row, int col);
static t_error	save_home(t_game *game, int row, int col);
...
static t_error	save_penguin(t_game *game, int row, int col)
{
	...
	if (!game->initial_pos)
		return (NO_ALLOCATION); */
	game->initial_row/* initial_pos->row */ = row;
	game->initial_col/* initial_pos->col */ = col;
