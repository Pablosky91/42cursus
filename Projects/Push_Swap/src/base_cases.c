...
/*   Updated: 2024/07/24 19:02:49 by pdel-olm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static t_error	base_case_three_bot_a(t_data *data, t_half *half);
static t_error	base_case_three_top_b(t_data *data, t_half *half);
static t_error	base_case_three_bot_b(t_data *data, t_half *half);
...
t_error	base_case_two(t_data *data, t_half *half, t_move *aux)
{
	...
	if (half->location == top_a)
		...
		aux = (t_move *)bot_b_10;
	while (*aux && !error_code)
		error_code = move(data, *aux++);
...
t_error	base_case_three(t_data *data, t_half *half)
{
	...
	if (half->location == top_a)
	{
		...
		while (*aux && !error_code)
		error_code = move(data, *aux++);//TODO return
	}
	else if (half->location == bot_a)
		error_code = base_case_three_bot_a(data, half);
	else if (half->location == top_b)
		error_code = base_case_three_top_b(data, half);
	else
		error_code = base_case_three_bot_b(data, half);
	return (error_code);//TODO return
}

/*
Particular case of base_case_three where the given half is in the bot_a location.
*/
t_error	base_case_three_bot_a(t_data *data, t_half *half)
{
...
*/
t_error	base_case_three_top_b(t_data *data, t_half *half)
{
...
*/
t_error	base_case_three_bot_b(t_data *data, t_half *half)
{
